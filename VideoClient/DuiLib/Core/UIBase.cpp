...
int CharConverter::WChar2MByte(const wchar_t* lpWChar, char* lpMByte)
{
	return WChar2MByteBase(lpWChar, lpMByte, CP_ACP);
}
int CharConverter::MByte2WChar(const char* lpMByte, wchar_t* lpWChar)
{
	return MByte2WCharBase(lpMByte, lpWChar, CP_ACP);
}
int CharConverter::WChar2Utf8(const wchar_t* lpWChar, char* lpUtf8)
{
	return WChar2MByteBase(lpWChar, lpUtf8, CP_UTF8);
}
int CharConverter::Utf82WChar(const char* lpUtf8, wchar_t* lpWChar)
{
	return MByte2WCharBase(lpUtf8, lpWChar, CP_UTF8);
}

int CharConverter::MByte2Utf8(const char* lpChar, char* lpUtf8)
{
	wchar_t plWchar[256] = { 0 };
	MByte2WChar(lpChar, plWchar);

	return WChar2Utf8(plWchar, lpUtf8);
}

int CharConverter::Utf82MByte(const char* lpUtf8, char* lpChar)
{
	wchar_t plWchar[256] = { 0 };
	Utf82WChar(lpUtf8, plWchar);

	return WChar2MByte(plWchar, lpChar);
}


int CharConverter::WChar2MByteBase(const wchar_t* lpWChar, char* lpMByte, UINT uPageCode)
{
	int nLen = WideCharToMultiByte(uPageCode, 0, lpWChar, -1, NULL, 0, NULL, NULL);
	if (lpMByte == NULL)
		return nLen;

	char *pcMByte = new char[nLen];
	memset(pcMByte, 0, nLen);

	WideCharToMultiByte(uPageCode, 0, lpWChar, -1, pcMByte, nLen, NULL, NULL);
	strcpy_s(lpMByte, nLen, pcMByte);

	delete[] pcMByte;

	return nLen;
}
int CharConverter::MByte2WCharBase(const char* lpMByte, wchar_t* lpWChar, UINT uPageCode)
{
	int nLen = MultiByteToWideChar(uPageCode, 0, lpMByte, -1, NULL, 0);
	if (lpWChar == NULL)
		return nLen;

	WCHAR *pWChar = new WCHAR[nLen];
	memset(pWChar, 0, sizeof(WCHAR) * nLen);

	MultiByteToWideChar(uPageCode, 0, lpMByte, -1, pWChar, nLen);
	wcscpy_s(lpWChar, nLen, pWChar);
...
int CharConverter::RTrim(char* lpChar)
{
	char* pTmp = lpChar + strlen(lpChar) - 1;
	while (*pTmp == 0x20)
	{
		*pTmp-- = 0x00;
	}
...
int CharConverter::LTrim(char* lpChar)
{
	return 0;
}
